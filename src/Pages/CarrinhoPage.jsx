...
const Carrinho = () => {
  ...
  const numeroWhatsApp = "5511986414262"; 
  
  const mensagem = carrinho.map((item) => {
    return `Produto: ${item.nome} | Quantidade: ${item.quantidade} | Preço: R$ ${item.preco.toFixed(2)}`
  }).join("\n");
      ...
      {carrinho.length === 0 ? (
        ...
        carrinho.map((item) => (
          <ProdutoCard key={item.id}>
            ...
            <Acoes>
              <Botao onClick={() => alterarQuantidade(item.id, "aumentar")}>+</Botao>
              <Botao onClick={() => alterarQuantidade(item.id, "diminuir")}>-</Botao>
